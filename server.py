...
from aiohttp import web  # keep-alive endpoint (still on port 8080)
...
# --- reduce noisy tracebacks from websockets library ---
# set websockets lib logging to WARNING to avoid repeated stack traces from failed handshake
...
async def broadcast(message, exclude_ws=None):
    # iterate over a snapshot to avoid runtime-dict-changes while sending
    for ws in list(clients.keys()):
        if ws != exclude_ws:
            try:
                await ws.send(message)
            except Exception:
                # avoid printing traceback for each failed send; just remove dead client
                clients.pop(ws, None)
                # try to remove from users if present
                for name, w in list(users.items()):
                    if w == ws:
                        users.pop(name, None)
                        log(f"Removed dead socket for user {name}", "warning")
...
# --- process_request to safely answer non-websocket HTTP probes ---
async def process_request(path, request_headers):
    """
    If an incoming connection is NOT a websocket upgrade (e.g. GET/HEAD),
    return a simple OK response (status, headers, body) so probes won't
    cause handshake exceptions.
    """
    # If "Upgrade" header exists and equals "websocket", let websockets handle it by returning None.
...
async def handler(ws, path):
    try:
        ...
        # log internal error more cleanly (no huge traceback echoed to stdout)
        log(f"Handler error for {locals().get('username','<unknown>')}: {e}", "error")
    finally:
        if ws in clients:
            uname, _ = clients.pop(ws)
            users.pop(uname, None)
            warn_count.pop(uname, None)
            muted_users.discard(uname)
            log(f"User disconnected: {uname}")
            await broadcast(Fore.YELLOW + f"User {uname} left the chat." + Style.RESET_ALL)
            await send_online_users()

# ✅ HTTP keep-alive endpoint (unchanged) - keep on port 8080 if you want separate healthchecks
